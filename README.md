...
## ⚠️ 紧急修正：关于 "框架预设"
**请注意：在配置部署时，不要选择 "VitePress"！那个是写文档用的！**
**请直接选择 "None" (无)，然后手动填写命令，详见下方第三步。**
...
7. 点击 **"Save and Deploy" (保存并部署)**。
   *等待约 1-2 分钟，直到看到 "Success!"。此时打开网站可能还无法保存数据，因为还没绑定数据库。*
...
## 如何使用
1. 打开部署好的网址。
2. 输入你在第三步设置的密码 (`AUTH_SECRET`)。
3. 点击 **"Add Source"** 添加你的接口（支持 JSON, M3U, 爬虫 Jar）。
4. 首页最上方那个醒目的 **Subscription Link** 就是你的专属订阅地址。
5. 把这个地址填入 TVBox 配置即可。以后有源失效，只需要在网页上修改，TVBox 那边会自动同步。

## 常见错误排查
*   **网站打开显示 404**:
    *   检查第三步：Output directory 是不是填错了？必须是 `dist`。
    *   检查是否选错了框架预设（比如选了 VitePress 就会导致这个问题）。
*   **登录后添加源报错 / 无法保存**:
    *   检查第五步：KV 是否绑定了？
    *   检查 KV 的 Variable name 是否严格填写的 `TVBOX_KV`？
    *   绑定完 KV 后，**必须执行第六步（重试部署）**，否则旧代码不认识新数据库。
